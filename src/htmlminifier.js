...
function collapseWhitespaceAll(str) {
  ...
  return str && str.replace(/[ \n\r\t\f\xA0]+/g, spaces => spaces === '\t' ? '\t' : spaces.replace(/(^|\xA0+)[^\xA0]+/g, '$1 '));
}

function collapseWhitespace(
  str,
  { preserveLineBreaks, conservativeCollapse },
  trimLeft,
  trimRight,
  collapseAll
) {
  ...
  if (trimLeft) {
    // Non-breaking space is specifically handled inside the replacer function here:
    str = str.replace(/^[ \n\r\t\f\xA0]+/, spaces => {
      const conservative = !lineBreakBefore && conservativeCollapse;
  ...
  if (trimRight) {
    // Non-breaking space is specifically handled inside the replacer function here:
    str = str.replace(/[ \n\r\t\f\xA0]+$/, spaces => {
      const conservative = !lineBreakAfter && conservativeCollapse;
      if (conservative && spaces === '\t') {
        return '\t';
      }
...
function isIgnoredComment(text, { ignoreCustomComments }) {
  for (let i = 0, len = ignoreCustomComments.length; i < len; i++) {
    if (ignoreCustomComments[i].test(text)) {
      return true;
    }
  }
  return false;
...
function isEventAttribute(attrName, { customEventAttributes }) {
  const patterns = customEventAttributes;
  if (patterns) {
...
function cleanAttributeValue(tag, attrName, attrValue, options, attrs) {
  if (isEventAttribute(attrName, options)) {
    ...
    attrValue = trimWhitespace(attrValue).split(/\s+,\s*|\s*,\s+/).map(candidate => {
      let url = candidate;
      let descriptor = '';
    ...
    attrValue = attrValue.replace(/\s+/g, '').replace(/[0-9]+\.[0-9]+/g, numString => // "0.90000" -> "0.9"
    // "1.0" -> "1"
...
function cleanConditionalComment(comment, options) {
  return options.processConditionalComments ? comment.replace(/^(\[if\s[^\]]+]>)([\s\S]*?)(<!\[endif])$/, (match, prefix, text, suffix) => prefix + minify(text, options, true) + suffix) : comment;
}
...
function processOptions(values) {
  ...
  Object.keys(values).forEach(key => {
    ...
    if (key === 'caseSensitive') {
      ...
      options.minifyCSS = (text, type) => {
        text = text.replace(/(url\s*\(\s*)("|'|)(.*?)\2(\s*\))/ig, (match, prefix, quote, url, suffix) => prefix + quote + options.minifyURLs(url) + quote + suffix);
      ...
      options.minifyJS = (text, inline) => {
        const start = text.match(/^\s*<!--.*/);
        const code = start ? text.slice(start[0].length).replace(/\n\s*-->\s*$/, '') : text;
        value.parse.bare_returns = inline;
        const result = terser.minify(code, value);
        if (result.error) {
...
function createSortFns(value, options, uidIgnore, uidAttr) {
  ...
  function attrNames(attrs) {
    return attrs.map(({ name }) => options.name(name));
  }
...
function minify(value, options, partialMarkup) {
  ...
  const customFragments = options.ignoreCustomFragments.map(({ source }) => source);
    ...
    value = value.replace(reCustomIgnore, match => {
      if (!uidAttr) {
        ...
        if (options.minifyCSS) {
          options.minifyCSS = (fn => (text, type) => {
        ...
        if (options.minifyJS) {
          options.minifyJS = (fn => (text, type) => fn(text.replace(uidPattern, (match, prefix, index) => {
            const chunks = ignoredCustomMarkupChunks[+index];
            return chunks[1] + uidAttr + index + chunks[2];
          }), type))(options.minifyJS);
        }
      }
      const token = uidAttr + ignoredCustomMarkupChunks.length;
  ...
  new HTMLParser(value, {
    ...
    chars(text, prevTag, nextTag) {
      ...
      if (uidPattern && options.collapseWhitespace && stackNoTrimWhitespace.length) {
        text = text.replace(uidPattern, (match, prefix, index) => ignoredCustomMarkupChunks[+index][0]);
      }
      currentChars += text;
      if (text) {
        hasChars = true;
    ...
    doctype(doctype) {
      buffer.push(options.useShortDoctype ? `<!doctype${options.removeTagWhitespace ? '' : ' '}html>` :
  ...
  return joinResultSegments(buffer, options, uidPattern ? str => str.replace(uidPattern, (match, prefix, index, suffix) => {
    ...
    return chunk;
  }) : identity, uidIgnore ? str => str.replace(new RegExp(`<!--${uidIgnore}([0-9]+)-->`, 'g'), (match, index) => ignoredMarkupChunks[+index]) : identity);
}
